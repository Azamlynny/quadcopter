...
//  delay(333);
  ...
  int M1Speed = yPosition;
  int M2Speed = 0;
  int M3Speed = 0;
  int M4Speed = 0;
  M1Speed = map(M1Speed, 511, 1023, 0, 180); // scales values to servo library (value between 0 and 180)
  M2Speed = map(M2Speed, 0, 1, 0, 180);
  M3Speed = map(M3Speed, 0, 1, 0, 180);
  M4Speed = map(M4Speed, 0, 1, 0, 180);
  ESC1.write(M1Speed);    // Send the signal to the ESC
  ESC2.write(M2Speed);    
  ESC3.write(M3Speed);  
  ESC4.write(M4Speed);   
  delay(100);
